...
const PreSubmit = forwardRef(
  ({ XHandle, className, handleSubmit, isSafe }, ref) => {
    ...
    const validateXhandle = useCallback(async (e) => {
      ...
      if (xHandle !== "") {
        if (XUserNameRegex.test(xHandle)) {
          ...
          if (isSafe) submitBtnRef.current.disabled = false;
        } else {
    ...
    return (
      <form
        ...
        className={cn(
          "mt-2 flex flex-row items-center justify-center gap-4",
          className,
        )}
      >
        <Input
